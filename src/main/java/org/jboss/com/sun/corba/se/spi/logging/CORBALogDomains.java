...
/**
 * Defines constants for all of the logging domains used in the ORB. Note that this is the suffix to the log domain. The
 * full domain is given by <code>javax.enterprise.resource.corba.{ORBId}.{Log domain}</code> where {ORBId} is the ORB ID
...
public abstract class CORBALogDomains
{
    ...
    /**
     * Log domain for the code used to handle any kind of invocation protocol. This includes client and server
     * delegates, client and server request dispatchers, service contexts, portable interceptors, and the GIOP protocol
     * (but not CDR representation of data).
     */
    public static final String RPC_PROTOCOL = "rpc.protocol";

    /**
     * Log domain for low-level transport details, which are independent of encoding and presentation details. This
     * includes selectors, acceptors, connections, connection management, and any other transport management functions.
     */
    ...
    /**
     * Log domain for creation, destruction, and state change of Object Adapters and related classes (e.g. POAManager).
     */
