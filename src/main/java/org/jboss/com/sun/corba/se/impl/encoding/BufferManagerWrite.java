...
public abstract class BufferManagerWrite
{
     ...
     * 
     * 1. Resize the bbwi.buf like the current implementation of CDROutputStream.grow.
     * 
     * 2. Collect the buffer for a later send: this.bufQ.put(bbwi); return new ByteBufferWithInfo(bbwi.length);
     * 
     * 3. Send buffer as fragment: Backpatch fragment size field in bbwi.buf. Set more fragments bit in bbwi.buf.
     * this.connection.send(bbwi); return reinitialized bbwi.buf with fragment header
     * 
     ...
     * 
     * Case: overflow was never called (bbwi.buf contains complete message). Backpatch size field. If growing or
     * collecting: this.bufQ.put(bbwi). this.bufQ.iterate // However, see comment in getBufferQ
     ...
     * this.connection.send(fragment) If streaming: backpatch fragment size field in bbwi.buf. Set no more fragments
     * bit. this.connection.send(bbwi).
     */
    ...
    // XREVISIT - Currently a java.lang.Object during the rip-int-generic transition. Should eventually become a
    // GIOPOutputObject.
