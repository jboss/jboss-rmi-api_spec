...
public final class ORBUtility
{
    ...
    private static boolean isSystemExceptionTypeCode(TypeCode type, ORB orb)
    {
        ...
        try
        {
            return (type.kind().value() == TCKind._tk_except && type.member_count() == 3
                    ...
                    && type.member_type(1).equal(systemExceptionMembers[1].type) && type.member_type(2).equal(
    ...
    /**
     * Returns true if the given ORB could accurately be determined to be a Kestrel or earlier ORB. Note: If passed the
    ...
    /**
     * Returns true if it was accurately determined that the remote ORB is a foreign (non-JavaSoft) ORB. Note: If passed
     * the ORBSingleton, this will return false.
     */
    ...
    /**
     * Get the class name corresponding to a particular repository Id. This is used by the system to unmarshal
     * (instantiate) the appropriate exception class for an marshaled as the value of its repository Id.
     * 
     * @param repositoryId
    ...
    public static byte getEncodingVersion(ORB orb, IOR ior)
    {
        ...
        if (orb.getORBData().isJavaSerializationEnabled())
        {
            ...
            Iterator<Object> iter = profTemp.iteratorById(ORBConstants.TAG_JAVA_SERIALIZATION_ID);
            if (iter.hasNext())
            {
    ...
    public static String repositoryIdOf(String name)
    {
        ...
        id = exceptionRepositoryIds.get(name);
        if (id == null)
    ...
    private static final Hashtable<String, String> exceptionClassNames = new Hashtable<String, String>();

    private static final Hashtable<String, String> exceptionRepositoryIds = new Hashtable<String, String>();
    ...
    /**
     * Parse a version string such as "1.1.6" or "jdk1.2fcs" into a version array of integers {1, 1, 6} or {1, 2}. A
     * string of "n." or "n..m" is equivalent to "n.0" or "n.0.m" respectively.
     */
    ...
    // Return a compressed representation of the thread name. This is particularly useful on the server side, where
    // there are many SelectReaderThreads, and we need a short unambiguous name for such threads.
    ...
    {
        System.out.println(compressClassName(obj.getClass().getName()) + "(" + getThreadName(Thread.currentThread())
                + "): " + msg);
    }

    public static synchronized void dprint(String className, String msg)
    {
        System.out.println(compressClassName(className) + "(" + getThreadName(Thread.currentThread()) + "): " + msg);
    }
    ...
    {
        System.out.println(compressClassName(caller.getClass().getName()) + '(' + Thread.currentThread() + "): " + msg);
     ...
     * 
     * Duplicated from util/Utility for Pure ORB reasons. There are two reasons for this:
     * 
     * 1) We can't introduce dependencies on the util version from outside of the io/util packages since it will not
     * exist in the pure ORB build running on JDK 1.3.x.
     * 
    ...
    /**
     * This method obtains an IOR from a CORBA object reference. It will return null if obj is a local object, a null
     * object, or an object implemented by a different ORB. It will throw BAD_OPERATION if obj is an unconnected
     * RMI-IIOP object.
     * 
     * @return IOR the IOR that represents this objref. This will never be null.
     * @exception BAD_OPERATION
     *                (from oi._get_delegate) if obj is a normal objref, but does not have a delegate set.
     * @exception BAD_PARAM
    ...
    public static IOR getIOR(org.omg.CORBA.Object obj)
    {
        ...
        if (StubAdapter.isStub(obj))
        {
            ...
            if (del instanceof CorbaClientDelegate)
            {
                ...
                {
                    // This is our code, but the ContactInfoList is not a CorbaContactInfoList. This should not happen,
                    // because we are in the CORBA application of the DCSA framework. This is a coding error, and thus
                    // an INTERNAL exception should be thrown.
                    // XXX needs minor code
                    throw new INTERNAL();
                }
            }

            // obj is implemented by a foreign ORB, because the Delegate is not a ClientDelegate.
            // XXX this case could be handled by marshalling and unmarshalling. However, object_to_string cannot be used
            // here, as it is implemented with getIOR. Note that this will require access to an ORB, so that we can
            // create streams as needed. The ORB is available simply as io._orb().
     ...
     * @return IOR the IOR that represents this objref. This will never be null.
     * @exception BAD_OPERATION
     *                if the object could not be connected, if a connection attempt was needed.
     * @exception BAD_PARAM
     *                if obj is a local object, or else was created by a foreign ORB.
     */
