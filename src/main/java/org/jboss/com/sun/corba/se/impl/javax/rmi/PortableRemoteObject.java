 ...
 * interface and then use the exportObject method to register themselves as a server object. The toStub method takes a
 * server implementation and returns a stub that can be used to access that server object. The connect method makes a
 * Remote object ready for remote communication. The unexportObject method is used to deregister a server object,
 * allowing it to become available for garbage collection. The narrow method takes an object reference or abstract
 * interface type and attempts to narrow it to conform to the given interface. If the operation is successful the result
 * will be an object of the specified type, otherwise an exception will be thrown.
 */
public class PortableRemoteObject implements javax.rmi.CORBA.PortableRemoteObjectDelegate
{

    /**
     * Makes a server object ready to receive remote calls. Note that subclasses of PortableRemoteObject do not need to
     * call this method, as it is called by the constructor.
     * 
     * @param obj
     ...
     * 
     * @param obj
     *            the server object for which a stub is required. Must either be a subclass of PortableRemoteObject or
     *            have been previously the target of a call to {@link #exportObject}.
    ...
    public java.lang.Object narrow(java.lang.Object narrowFrom, java.lang.Class<?> narrowTo) throws ClassCastException
    {
        ...
        try
        {
            ...
            // Is narrowTo an interface that might be implemented by a servant running on iiop?
     ...
     * making an explicit call. See the {@link Stub#connect} method for more information.
     * 
     * @param target
     ...
     *             if <code>source</code> is not connected or if <code>target</code> is already connected to a different
     *             ORB than <code>source</code>.
     */
    public void connect(Remote target, Remote source) throws RemoteException
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    /*
                     * loadTie always succeeds for dynamic RMI-IIOP // No, can we get a tie for it? If not, // assume
                     * that source is a JRMP object... if (Utility.loadTie(source) != null) { // Yes, so it is an iiop
                     * object which // has not been exported... throw new RemoteException(
        ...
        else
        {
            ...
            {
                /*
                 * loadTie always succeeds for dynamic RMI-IIOP if (Utility.loadTie(target) != null) { throw new
        ...
        if (!targetIsIIOP)
        {
            // Yes. Do we have an ORB from the source object? If not, we're done - there is nothing to do to connect a
            // JRMP object. If so, it is an error because the caller mixed JRMP and IIOP...
            if (orb != null)
            {
        ...
        {
            // The target object is IIOP. Make sure we have a valid ORB from the source object...
            if (orb == null)
            {
