...
public final class OSFCodeSetRegistry
{
    ...
    //
    // Please see the individual Entry definitions for more details.
    ...
    /**
     * An entry in the OSF registry which allows users to find out the equivalent Java character encoding name as well
     * as some other facts from the registry.
     */
    public final static class Entry
    {
        ...
        /**
         * Returns the Java equivalent name. If the encoding has an optional byte order marker, this name will map to
         * the Java encoding that includes the marker.
         */
        ...
        /**
         * Is this a fixed or variable width code set? (In CORBA terms, "non-byte-oriented" or a "byte-oriented" code
         * set, respectively)
         */
    ...
    /**
     * UTF-16 as specified in the OSF registry has an optional byte order marker. UTF-16BE and UTF-16LE are not in the
     * OSF registry since it is no longer being developed. When the OMG switches to the IANA registry, these can be
     * public. Right now, they're used internally by CodeSetConversion.
     */
     ...
     * 
     * In the resolution of issue 3405b, UTF-16 defaults to big endian, so doesn't have to have a byte order marker.
     * Unfortunately, this has to be a special case for compatibility.
     */
    public static final Entry UTF_16 = new Entry("UTF-16", UTF_16_VALUE, true, 4);

    /**
     * Fallback char code set. Also the code set for char data in encapsulations. However, since CORBA says chars are
     * only one octet, it is really the same as Latin-1.
     */
    public static final Entry UTF_8 = new Entry("UTF-8", UTF_8_VALUE, false, 6);

    /*
     * At least in JDK 1.3, UCS-2 isn't one of the mandatory Java character encodings. However, our old ORBs require
     * what they call UCS2, even though they didn't necessarily do the correct encoding of it.
     * 
     * This is a special case for our legacy ORBs, and put as the last thing in our conversion list for wchar data.
     * 
    ...
    /**
     * This is the encoding older JavaSoft ORBs advertised as their CORBA char code set. Actually, they took the lower
     * byte of the Java char. This is a 7-bit encoding, so they were really sending ISO8859-1.
     */
    public static final Entry ISO_646 = new Entry("US-ASCII", ISO_646_VALUE, true, 1);

    /**
     * Given an OSF registry value, return the corresponding Entry. Returns null if an Entry for that value is
     * unavailable.
     */
