...
import org.jboss.com.sun.corba.se.impl.presentation.rmi.StubConnectImpl;
...
public class StubDelegateImpl implements javax.rmi.CORBA.StubDelegate
{
    ...
    {
        // If the Stub is not connected to an ORB, BAD_OPERATION exception will be raised by the code below.
        if (ior == null)
    ...
    /**
     * Compares two stubs for equality. Returns <code>true</code> when used to compare stubs that represent the same
     * remote object, and <code>false</code> otherwise.
     * 
     * @param obj
    ...
    public boolean equals(javax.rmi.CORBA.Stub self, java.lang.Object obj)
    {
        ...
        // hashCodes being the same does not mean equality. The stubs still could be pointing to different IORs. So, do
        // a literal comparison. Apparently the ONLY way to do this (other than using private reflection) toString,
        // because it is not possible to directly access the StubDelegateImpl from the Stub.
    ...
    /**
     * Returns a string representation of this stub. Returns the same string for all stubs that represent the same
     * remote object.
     * 
     ...
     * stream. If an unconnected stub is passed to an ORB stream for marshalling, it is implicitly connected to that
     * ORB. Application code should not call this method directly, but should call the portable wrapper method
     * {@link javax.rmi.PortableRemoteObject#connect}.
     * 
     * @param orb
     *            the ORB to connect to.
     * @exception RemoteException
     *                if the stub is already connected to a different ORB, or if the stub does not represent an exported
     *                remote or local object.
     */
     ...
     * 
     * @serialData The length of the IOR type ID (int), followed by the IOR type ID (byte array encoded using
     *             ISO8859-1), followed by the number of IOR profiles (int), followed by the IOR profiles. Each IOR
     *             profile is written as a profile tag (int), followed by the length of the profile data (int), followed
