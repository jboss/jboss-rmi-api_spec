...
public final class Utility
{
    ...
    /**
     * Ensure that stubs, ties, and implementation objects are 'connected' to the runtime. Converts implementation
     ...
     * @param convertToStub
     *            true if implementation types should be converted to Stubs rather than just org.omg.CORBA.Object.
    ...
    public static Object autoConnect(Object obj, ORB orb, boolean convertToStub)
    {
        ...
        if (StubAdapter.isStub(obj))
        {
            ...
            catch (BAD_OPERATION okay)
            {
                ...
                {
                    // The stub could not be connected because it has an invalid IOR...
        ...
        if (obj instanceof Remote)
        {
            ...
            {
                // This is an implementation object which has not been exported to IIOP OR is a JRMP stub or
                // implementation object which cannot be marshalled into an ORB stream...
        ...
        synchronized (tieCache)
        {
            ...
            if (it == null)
            {
                ...
                try
                {
                    ...
                    // If we don't have a valid tie at this point, walk up the parent chain until we either load a tie
                    // or encounter PortableRemoteObject or java.lang.Object...
            ...
            {
                // Yes, return a new instance or fail again if it was a miss last time...
                if (it != CACHE_MISS)
                {
                    try
                    {
    ...
    {
        return org.jboss.com.sun.corba.se.spi.orb.ORB.getStubFactoryFactory().getTie(theClass);
    }
     ...
     */
    static Class<?> loadClassOfType(String className, String remoteCodebase, ClassLoader loader, Class<?> expectedType,
            ClassLoader expectedTypeClassLoader) throws ClassNotFoundException
    {
        ...
        try
        {
            ...
            try
            {
                // If-else is put here for speed up of J2EE. According to the OMG spec, the if clause is not dead code.
                // It can occur if some compiler has allowed generation into org.omg.stub hierarchy for non-offending
                // classes. This will encourage people to produce non-offending class stubs in their own hierarchy.
        ...
        // If no class was loaded, or if the loaded class is not of the correct type, make a further attempt to load the
        // correct class using the classloader of the expected type.
            ...
            Class<?> relatedType, ClassLoader relatedTypeClassLoader) throws ClassNotFoundException
    {
        ...
        // the correct class using the classloader of the related type.
        // _REVISIT_ Is this step necessary, or should the Util,loadClass algorithm always produce a valid class if the
        // setup is correct? Does the OMG standard algorithm need to be changed to include this step?
        if (loadedClass == null
                || (loadedClass.getClassLoader() != null && loadedClass.getClassLoader().loadClass(
                        relatedType.getName()) != relatedType))
        {
    ...
    public static ValueFactory getFactory(Class<?> clazz, String codebase, ORB orb, String repId)
    {
        ...
        // if earlier search found a non-default factory, or the same default factory that loadClassForClass would
        // return, bale out now...
        if (factory != null
                && (!factory.getClass().getName().equals(className + "DefaultFactory") || (clazz == null && codebase == null)))
            return factory;
     ...
     * @param remoteCodebase the codebase to use. May be null.
     * 
     * @param onlyMostDerived if true, will fail if cannot load a stub for the first repID in the tie. If false, will
     * walk all repIDs.
     * 
            ...
            boolean onlyMostDerived)
    {
        ...
        synchronized (tieToStubCache)
        {
            ...
            {
                // Yes, is it a stub? If not, it was a miss last time, so return null again...
                if (cached != CACHE_MISS)
                {
                    // It's a stub.
                    entry = (StubEntry) cached;

                    // Does the cached stub meet the requirements of the caller? If the caller does not require the most
                    // derived stub and does not require a specific stub type, we don't have to check any further
                    // because the cached type is good enough...
                    if (!entry.mostDerived && onlyMostDerived)
                    {
                        // We must reload because we do not have the most derived cached already... The stubFactory arg
                        // must be null here to force onlyMostDerived=true to work correctly.
                        entry = loadStubAndUpdateCache(tie, null, remoteCodebase, true);
                    }
                    else if (stubFactory != null
                            && !StubAdapter.getTypeIds(entry.stub)[0].equals(stubFactory.getTypeIds()[0]))
                    {
                        // We do not have exactly the right stub. First, try to upgrade the cached stub by forcing it to
                        // the most derived stub...
                        entry = loadStubAndUpdateCache(tie, null, remoteCodebase, true);
                    ...
                    else
                    {
                        ...
                        try
                        {
                            StubAdapter.getDelegate(entry.stub);
                        }
                        catch (Exception e2)
                        {
    ...
    /*
     * Load an RMI-IIOP Stub given a Tie, but do not look in the cache. This method must be called with the lock held
     * for tieToStubCache.
     * 
     * @param tie the tie.
     * 
            ...
            String remoteCodebase, boolean onlyMostDerived)
    {
        ...
        else
        {
            ...
            {
                // This will throw an exception if the tie is not a Servant. XXX Handle this better?
            ...
            else
            {
                ...
                for (int i = 0; i < ids.length; i++)
                {
                    ...
                    try
                    {
                        PresentationManager.StubFactoryFactory stubFactoryFactory = org.jboss.com.sun.corba.se.spi.orb.ORB
        ...
        else
        {
            if (tieIsStub)
            {
                ...
                {
                    // The tie does not have a delegate set, so stash this tie away using the stub as a key so that
                    // later, when the stub is connected, we can find and connect the tie as well...
                    synchronized (stubToTieCache)
                    {
    ...
    /*
     * If we loadStub(Tie,...) stashed away a tie which was not connected, remove it from the cache and return it.
     */
    ...
    public static String stubNameFromRepID(String repID)
    {
        ...
        // Convert the typeid to a RepositoryId instance, get the className and mangle it as needed...
    ...
    /*
     * Load an RMI-IIOP Stub class. This is used in the StaticStubFactoryFactory code.
     */
    public static Class<?> loadStubClass(String repID, String remoteCodebase, Class<?> expectedType)
            throws ClassNotFoundException
    {
        // Get the repID and check for "" special case. We should never be called with it (See CDRInputStream and the
        // loadStub() method)...
        ...
        // Get the stubname from the repID and load the class. If we have a valid 'sender', fall back to using its
        // codebase if we need to...
    ...
    public static String idlStubName(String className)
    {
        ...
        if (index > 0)
        {
            result = className.substring(0, index + 1) + STUB_PREFIX + className.substring(index + 1) + IDL_STUB_SUFFIX;
        }
        else
        {
