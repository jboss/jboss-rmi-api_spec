...
{
    // Indicates whether the header is padded. In GIOP 1.2 and above,/ the body must be aligned on an 8-octet boundary,
    // and so the header is padded appropriately. However, if there is no body to a request or reply message, there is
    // no header padding, in the unfragmented case.
    ...
    // the mark and reset methods have been overridden to remember the headerPadding flag.
    ...
    // Template method
    // This method has been overriden to ensure that the duplicated stream inherits the headerPadding flag, in case of
    // GIOP 1.2 and above, streams.
    ...
    protected void alignAndCheck(int align, int n)
    {
        ...
        // headerPadding bit is set by read method of the RequestMessage_1_2 or ReplyMessage_1_2 classes. When set, the
        // very first body read operation (from the stub code) would trigger an alignAndCheck method call, that would in
        // turn skip the header padding that was inserted during the earlier write operation by the sender. The padding
        // ensures that the body is aligned on an 8-octet boundary, for GIOP versions 1.2 and beyond.
        ...
        // In GIOP 1.2, a fragment may end with some alignment padding (which leads to all fragments ending perfectly on
        // evenly divisible 8 byte boundaries). A new fragment never requires alignment with the header since it ends on
        // an 8 byte boundary.
    ...
    {
        // In GIOP 1.2, a wchar is encoded as an unsigned octet length followed by the octets of the converted wchar.
    ...
    {
        // In GIOP 1.2, wstrings are not terminated by a null. The length is the number of octets in the converted
        // format. A zero length string is represented with the 4 byte length value of 0.
