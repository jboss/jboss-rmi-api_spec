...
public class IIOPProfileImpl extends IdentifiableBase implements IIOPProfile
{
    ...
    public IIOPProfileImpl(ORB orb, org.omg.IOP.TaggedProfile profile)
    {
        ...
        EncapsInputStream istr = new EncapsInputStream(orb, profile.profile_data, profile.profile_data.length);
        istr.consumeEndian();
        init(istr);
    }
    ...
    private void init(InputStream istr)
    {
        ...
        // If there is no codebase in this IOR and there IS a java.rmi.server.codebase property set, we need to update
        // the IOR with the local codebase. Note that there is only one instance of the local codebase, but it can be
        // safely shared in multiple IORs since it is immutable.
        if (uncachedGetCodeBase() == null)
        {
            ...
            // Whether codebase is null or not, we have it, and so getCodebase ned never call uncachedGetCodebase.
            cachedCodebase = true;
        }
    }
    ...
    {
        Iterator<Object> iter = proftemp.iteratorById(TAG_JAVA_CODEBASE.value);
     ...
     * to servants outside of an invocation. XXX revisit: do we want this at all? If we do, it might move to the
    ...
    public java.lang.Object getServant()
    {
        ...
        {
            // Could not find the OA, so just return null. This usually happens when POAs are being deleted, and the POA
            // always return null for getLocalServant anyway.
