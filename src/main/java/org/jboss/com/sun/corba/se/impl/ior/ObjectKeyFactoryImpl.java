...
public class ObjectKeyFactoryImpl implements ObjectKeyFactory
{
    ...
    // Format changes: all object keys have version string; POA key format is changed.
    public static final int JAVAMAGIC_NEWER = MAGIC_BASE + 2;

    public static final int MAX_MAGIC = JAVAMAGIC_NEWER;

    // Beginning in JDK 1.3.1_01, we introduced changes which required the ability to distinguish between JDK 1.3.1 FCS
    // and the patch versions. See OldJIDLObjectKeyTemplate.
    ...
    private ObjectKeyTemplate create(InputStream is, Handler handler, OctetSeqHolder osh)
    {
        ...
        if (oktemp == null)
            // If we did not successfully construct a oktemp, reset the stream so that WireObjectKeyTemplate can
            // correctly construct the object key.
            try
            {
                is.reset();
            }
