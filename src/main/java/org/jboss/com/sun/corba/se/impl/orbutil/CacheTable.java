    ...
    private boolean put_table(java.lang.Object key, int val)
    {
        ...
        for (Entry e = map[index]; e != null; e = e.next)
        {
            if (e.key == key)
            {
                ...
                // if we get here we are trying to put in the same key/val pair this is a no-op, so we just return
                return false;
            }
        }
        // this means the key is not present in our table then it shouldnt be present in our reverse table either
