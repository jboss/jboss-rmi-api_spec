 ...
 * NOTE:
 * 
 * Before using the stream for valuetype unmarshaling, one must call performORBVersionSpecificInit().
 */
            ...
            org.omg.CORBA.portable.ValueInputStream
{
    ...
    // We can move this out somewhere later. For now, it serves its purpose to create a concrete CDR delegate based on
    // the GIOP version.
    ...
    // Required for the case when a ClientResponseImpl is created with a SystemException due to a dead server/closed
    // connection with no warning. Note that the stream will not be initialized in this case.
    //
    ...
    public final java.io.Serializable read_value(String rep_id)
    {
    ...
    // org.jboss.com.sun.corba.se.impl.encoding.MarshalInputStream
    ...
    // This should be overridden by any stream (ex: IIOPInputStream) which wants to read values. Thus,
    // TypeCodeInputStream doesn't have to do this.
    public CodeBase getCodeBase()
    {
        return null;
    }

    // Use Latin-1 for GIOP 1.0 or when code set negotiation was not performed.
    protected CodeSetConversion.BTCConverter createCharBTCConverter()
    {
        return CodeSetConversion.impl().getBTCConverter(OSFCodeSetRegistry.ISO_8859_1, impl.isLittleEndian());
    }

    // Subclasses must decide what to do here. It's inconvenient to make the class and this method abstract because of
    // dup().
    ...
    /**
     * This must be called after determining the proper ORB version, and setting it on the stream's ORB instance. It can
     * be called after reading the service contexts, since that is the only place we can get the ORB version info.
     * 
     * Trying to unmarshal things requiring repository IDs before calling this will result in NullPtrExceptions.
     */
    public void performORBVersionSpecificInit()
    {
        // In the case of SystemExceptions, a stream is created with its default constructor (and thus no impl is set).
        if (impl != null)
