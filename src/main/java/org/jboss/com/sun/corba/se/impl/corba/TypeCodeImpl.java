...
public final class TypeCodeImpl extends TypeCode
{
    ...
    // typecode encodings have three different categories that determine how the encoding should be done.
    ...
    // Maps TCKind values to names. This is also used in AnyImpl.
    static final String[] kindNames = {"null", "void", "short", "long", "ushort", "ulong", "float", "double",
            "boolean", "char", "octet", "any", "typecode", "principal", "objref", "struct", "union", "enum", "string",
            "sequence", "array", "alias", "exception", "longlong", "ulonglong", "longdouble", "wchar", "wstring",
            "fixed", "value", "valueBox", "native", "abstractInterface"};
    ...
    // value type
    // _REVISIT_ We might want to keep references to the ValueMember classes passed in at initialization instead of
    // copying the relevant data. Is the data immutable? What about StructMember, UnionMember etc.?
    ...
    private void setId(String newID)
    {
        ...
        // check whether return value != this which would indicate that the repository id isn't unique.
    }
                ...
                case COMPLEX :
                    ...
                    switch (_kind)
                    {
                        ...
                        case TCKind._tk_value : {
                            ...
                            TypeCode tccb = tc.concrete_base_type();
                            if ((_concrete_base == null && tccb != null) || (_concrete_base != null && tccb == null)
     ...
     * See AnyImpl read_value and write_value for usage. The state of this TypeCodeImpl instance isn't changed, only
     * used by the Any to do the correct copy.
     */
    ...
    protected void copy(org.omg.CORBA.portable.InputStream src, org.omg.CORBA.portable.OutputStream dst)
    {
        switch (_kind)
        {
            ...
            case TCKind._tk_union :
                /*
                 * _REVISIT_ More generic code? { Any discriminator = new AnyImpl(_orb); discriminator.read_value(src,
                 * _discriminator); discriminator.write_value(dst); int labelIndex =
                 * currentUnionMemberIndex(discriminator); if (labelIndex == -1) { // check if label has not been found
                 * if (_defaultIndex == -1) // throw exception if default was not expected throw new MARSHAL(); else //
                 * must be of the default branch type _memberTypes[_defaultIndex].copy(src, dst); } else {
