        ...
        try
        {
            ...
            catch (org.omg.CORBA.BAD_OPERATION err)
            {
                if (ior == null)
                {
                    ...
                    // Is the tie already connected? If it is, check that it's connected to the same ORB, otherwise
                    // connect it.
                    ORB existingOrb = orb;
                    try
                    {
                ...
                {
                    // ior is initialized, so convert ior to an object, extract the delegate, and set it on ourself
