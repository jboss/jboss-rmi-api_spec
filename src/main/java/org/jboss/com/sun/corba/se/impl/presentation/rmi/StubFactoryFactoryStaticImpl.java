...
public class StubFactoryFactoryStaticImpl extends StubFactoryFactoryBase
{
            ...
            String remoteCodeBase, Class<?> expectedClass, ClassLoader classLoader)
    {
        ...
        // The old code was optimized to try to guess which way to load classes first. The real stub class name could
        // either be className or "org.omg.stub." + className. We will compute this as follows:
        // If stubName starts with a "forbidden" package, try the prefixed version first, otherwise try the non-prefixed
        // version first. In any case, try both forms if necessary.
        ...
        // algorithm always produce a valid class if the setup is correct? Does the OMG standard algorithm need to be
        // changed to include this step?
    ...
    public Tie getTie(Class<?> cls)
    {
        ...
        try
        {
            try
            {
                // _REVISIT_ The spec does not specify a loadingContext parameter for the following call. Would it be useful to pass one?
            ...
            {
                tieClass = Utility.loadClassForClass(PackagePrefixChecker.packagePrefix() + className,
