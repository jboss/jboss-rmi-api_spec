...
import org.jboss.com.sun.corba.se.impl.orb.ORBVersionImpl;
...
public class ORBVersionFactory
{
    ...
    {
        /*
         * Throwing an exception here would cause this version to be incompatible with future versions of the ORB, to
         * the point that this version could not even unmarshal objrefs from a newer version that uses extended
         * versioning. Therefore, we will simply treat all unknown versions as the latest version. if (value < 0) throw
         * new INTERNAL() ;
         */

        /**
         * Update: If we treat all unknown versions as the latest version then when we send an IOR with a PEORB version
         * to an ORB that doesn't know the PEORB version it will treat it as whatever its idea of the latest version is.
         * Then, if that IOR is sent back to the server and compared with the original the equality check will fail
         * because the versions will be different.
         * 
