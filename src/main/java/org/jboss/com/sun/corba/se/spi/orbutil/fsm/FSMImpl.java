...
import org.jboss.com.sun.corba.se.impl.orbutil.fsm.StateEngineImpl;

/**
 * This is the main class that represents an instance of a state machine using a state engine. It may be used as a base
 * class, in which case the guards and actions have access to the derived class.
 * 
...
public class FSMImpl implements FSM
{
     ...
     * <p>
     * Let S be the current state of the FSM. If there are guarded actions for S with input in, evaluate their guards
     * successively until all have been evaluted, or one returns a non-DISABLED Result.
     * <ol>
     * <li>If a DEFERED result is returned, retry the input
     * <li>If a ENABLED result is returned, the action for the guarded action is the current action
     * <li>Otherwise there is no enabled action. If S has a default action and next state, use them; otherwise use the
     * state engine default action (the next state is always the current state).
     * </ol>
