...
/**
 * Collection of classes, interfaces, and factory methods for CORBA code set conversion.
 * 
 * This is mainly used to shield other code from the sun.io converters which might change, as well as provide some basic
 * translation from conversion to CORBA error exceptions. Some extra work is required here to facilitate the way CORBA
 * says it uses UTF-16 as of the 00-11-03 spec.
 * 
 * REVISIT - Since the nio.Charset and nio.Charset.Encoder/Decoder use NIO ByteBuffer and NIO CharBuffer, the
 * interaction and interface between this class and the CDR streams should be looked at more closely for optimizations
 * to avoid unnecessary copying of data between char[] & CharBuffer and byte[] & ByteBuffer, especially
 * DirectByteBuffers.
 * 
 */
public class CodeSetConversion
{
     ...
     * 
     * 1) convert 2) Optional getNumBytes and/or getAlignment (if necessary) 3) getBytes (see warning)
     */
    ...
    /**
     * Implementation of CTBConverter which uses a nio.Charset.CharsetEncoder to do the real work. Handles translation
     * of exceptions to the appropriate CORBA versions.
     */
    ...
    /**
     * Implementation of BTCConverter which uses a sun.io.ByteToCharConverter for the real work. Handles translation of
     * exceptions to the appropriate CORBA versions.
     */
    private class JavaBTCConverter extends BTCConverter
    {
        ...
        /**
         * Utility method to find a CharsetDecoder in the cache or create a new one if necessary. Throws an INTERNAL if
         * the code set is unknown.
         */
    ...
    /**
     * Special converter for UTF16 since it's required to optionally support a byte order marker while the internal Java
     * converters either require it or require that it isn't there.
     * 
     * The solution is to check for the byte order marker, and if we need to do something differently, switch internal
     * converters.
     */
    private class UTF16BTCConverter extends JavaBTCConverter
    {
        ...
        /**
         * The current solution for dealing with UTF-16 in CORBA is that if our sun.io converter requires byte order
         * markers, and then we see a CORBA wstring/wchar without them, we switch to the sun.io converter that doesn't
         * require them.
         */
     ...
     * 
     * Because of the awkwardness with byte order markers and the possibility of using UCS-2, you must specify both the
     * endianness of the stream as well as whether or not to use byte order markers if applicable. UCS-2 has no byte
     * order markers. UTF-16 has optional markers.
     * 
     * If you select useByteOrderMarkers, there is no guarantee that the encoding will use the endianness specified.
     * 
     */
     ...
     * 
     * Returns the proper negotiated OSF character encoding number or CodeSetConversion.FALLBACK_CODESET.
     */
