...
{
    /**
     * JDK 1.3.1 FCS did not include a version byte at the end of its object keys. JDK 1.3.1_01 included the byte with
    ...
    public OldJIDLObjectKeyTemplate(ORB orb, int magic, int scid, InputStream is, OctetSeqHolder osh)
    {
         ...
         * version. JDK 1.3.1_01 had the value 1. If other patches are necessary which involve ORB versioning changes,
         * they should increment the patch version.
         * 
         * Note that if we see a value greater than 1 in this code, we will treat it as if we're talking to the most
         * recent ORB version.
         * 
         * WARNING: This code is sensitive to changes in CDRInputStream getPosition. It assumes that the CDRInputStream
         * is an encapsulation whose position can be compared to the object key array length.
         */
        if (magic == ObjectKeyFactoryImpl.JAVAMAGIC_NEW && osh.value.length > ((CDRInputStream) is).getPosition())
        {
