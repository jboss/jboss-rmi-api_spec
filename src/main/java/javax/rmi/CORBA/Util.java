...
public class Util
{
     ...
     * <tt>ClassLoader</tt>.
     * <LI>If the first step fails, and if <tt>remoteCodebase</tt> is non-null and <tt>useCodebaseOnly</tt> is false,
     * then call <tt>java.rmi.server.RMIClassLoader.loadClass(remoteCodebase, className)</tt>.
     ...
     * @param loader
     *            a <tt>ClassLoader</tt> that may be used to load the class if all other methods fail.
    ...
    /**
     * The <tt>isLocal</tt> method has the same semantics as the <tt>ObjectImpl._is_local</tt> method, except that it
     * can throw a <tt>RemoteException</tt>.
     * 
     ...
     *             The Java to IDL specification does not specify the conditions that cause a <tt>RemoteException</tt>
     *             to be thrown.
     */
